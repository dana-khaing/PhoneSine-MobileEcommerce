...
export default function Home() {
  ...
  return (
    <main className="brand-shell">
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 md:grid-cols-[1.08fr_0.92fr] md:items-center lg:gap-10 lg:py-24">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.35em] text-neutral-500">PhoneSine Mobile</p>
          <h1 className="mt-5 max-w-3xl text-4xl font-black leading-[0.98] tracking-tight text-neutral-950 sm:text-5xl md:text-7xl">{t("heroTitle")}</h1>
        ...
        <div className="brand-card relative overflow-hidden rounded-[2rem] p-5 sm:p-8">
      ...
      <section className="bg-neutral-950 py-14 text-white"><div className="mx-auto max-w-7xl px-4 sm:px-6"><h2 className="text-2xl font-black">{t("categories")}</h2><div className="mt-6 flex flex-wrap gap-3">{categories.map((category) => <Link key={category.id} href={`/categories/${category.slug}`} className="rounded-full border border-white/15 bg-white/10 px-5 py-3 text-sm font-semibold backdrop-blur hover:bg-white/20">{category.name}</Link>)}</div></div></section>
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-16"><div className="flex items-end justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-[0.25em] text-neutral-500">{t("justAdded")}</p><h2 className="text-3xl font-black">{t("latest")}</h2></div><Link href="/products" className="shrink-0 font-semibold underline">{t("viewAll")}</Link></div><div className="mt-7 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{products.map((product) => <StorefrontCard key={product.id} product={product} />)}</div></section>
      {recommended.length > 0 && <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14"><h2 className="text-2xl font-bold">{t("recommended")}</h2><div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{recommended.map((product) => <StorefrontCard key={product.id} product={product} />)}</div></section>}
      {recent.length > 0 && <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-14"><h2 className="text-2xl font-bold">{t("recent")}</h2><div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{recent.map((product) => <StorefrontCard key={product.id} product={product} />)}</div></section>}
    </main>
  );
}
