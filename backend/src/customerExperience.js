const crypto = require("crypto"); const express = require("express"); const { GiftCard, SupportTicket } = require("../models"); const { requireAdmin, requireAuth } = require("./authMiddleware");
...
router.get("/admin/tickets", requireAdmin, async (_req, res) => res.json(await SupportTicket.findAll({ order: [["createdAt", "DESC"]] })));
router.patch("/admin/tickets/:id", requireAdmin, async (req, res) => { const ticket = await SupportTicket.findByPk(req.params.id); if (!ticket) return res.status(404).send("Ticket not found"); await ticket.update({ status: req.body.status || ticket.status, adminReply: req.body.adminReply ?? ticket.adminReply }); res.json(ticket); });
router.post("/admin/gift-cards", requireAdmin, async (req, res) => { const balanceAmount = Number(req.body.balanceAmount); if (!Number.isInteger(balanceAmount) || balanceAmount < 1) return res.status(400).send("Positive gift card balance required"); res.status(201).json(await GiftCard.create({ code: crypto.randomBytes(8).toString("hex").toUpperCase(), balanceAmount, currency: req.body.currency || "gbp" })); });
